...
export class Tx {
  ...
  async sign(
    ...
    let wallet = new ethers.Wallet(privKey, this.client.config.provider);
    let address:string = await wallet.getAddress()
    ...
    let accountNumber = account.base_account.account_number??0;
    let sequence = account.base_account.sequence??0
    
    if (!stdTx.hasPubKey()) {
      const pubKey = Crypto.getPublicKeyFromPrivateKey(privKey, baseTx.pubkeyType);
      stdTx.setPubKey(pubKey, parseInt(sequence)??undefined);
    }
    const signature = Crypto.generateSignature(stdTx.getSignDoc(parseInt(accountNumber)??undefined, baseTx.chainId || this.client.config.chainId).serializeBinary(), privKey, baseTx.pubkeyType);
