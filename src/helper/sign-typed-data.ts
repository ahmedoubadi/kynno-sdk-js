...
import { rawEncode, solidityPack } from '../utils/ethereumjs-abi-utils';
...
function encodeField(
  ...
  if (types[type] !== undefined) {
    // @ts-ignore
    return ['bytes32',version === SignTypedDataVersion.V4 && value == null 
  ...
  if (type === 'bytes') {
    if (typeof value === 'number') {
      value = Utils.numberToBuffer(value);
...
function _typedSignatureHash(typedData: TypedDataV1): Buffer {
  ...
  const data = typedData.map(function (e) {
    ...
    return Utils.legacyToBuffer(e.value);
  });
...
>({
  ...
  if (Utils.isNullish(data)) {
    throw new Error('Missing data parameter');
  } else if (Utils.isNullish(privateKey)) {
  ...
  return Utils.concatSig(toBuffer(sig.v), sig.r, sig.s);
}
...
>({
  ...
  if (Utils.isNullish(data)) {
    throw new Error('Missing data parameter');
  } else if (Utils.isNullish(signature)) {
